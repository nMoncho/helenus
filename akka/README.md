...
// session: CassandraSession = akka.stream.alpakka.cassandra.scaladsl.CassandraSession@5711368a
...
// hotels: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(1942038577)))

val hotelById: Source[Hotel, NotUsed] =
  "SELECT * FROM hotels WHERE id = ?".toCQLAsync.prepare[String].as[Hotel].asReadSource("h1")
// hotelById: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(190021949)))
...
// insert: Sink[Hotel, Future[Done]] = Sink(SinkShape(FlatMapPrefix.in(1710413647)))
```
