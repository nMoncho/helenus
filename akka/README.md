...
// session: CassandraSession = akka.stream.alpakka.cassandra.scaladsl.CassandraSession@4ce3a5f6
...
// hotels: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(124133008)))

val hotelById: Source[Hotel, NotUsed] =
  "SELECT * FROM hotels WHERE id = ?".toCQLAsync.prepare[String].as[Hotel].asReadSource("h1")
// hotelById: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(1580930877)))
...
// insert: Sink[Hotel, Future[Done]] = Sink(SinkShape(FlatMapPrefix.in(1147922329)))
```
