...
// session: CassandraSession = akka.stream.alpakka.cassandra.scaladsl.CassandraSession@531b7669
...
// hotels: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(1728372939)))

val hotelById: Source[Hotel, NotUsed] =
  "SELECT * FROM hotels WHERE id = ?".toCQLAsync.prepare[String].as[Hotel].asReadSource("h1")
// hotelById: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(673175955)))
...
// insert: Sink[Hotel, Future[Done]] = Sink(SinkShape(FlatMapPrefix.in(2104431110)))
```
