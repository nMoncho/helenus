...
These are **derived from Apache Cassandra's own ANTLR grammars**, not hand-maintained.
They are generated by `tools/antlr-import/convert.py`, which mechanically strips
Cassandra's ANTLR3 grammars down to a recognizer-only ANTLR4 grammar (no AST, no
Cassandra classes). Do not edit the `.g4` files by hand; edit the converter and
re-run it. See `tools/antlr-import/README.md` for the import process, the pinned
Cassandra version (`tools/antlr-import/upstream/VERSION`), and the intentional
deviations from upstream. `CqlConformanceSpec` is the corpus that guards a re-import.
