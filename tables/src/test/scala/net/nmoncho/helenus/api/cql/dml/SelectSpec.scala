...
class SelectSpec extends AnyFlatSpec with Matchers {
  ...
  "Select" should "generate a SELECT for specific columns" in {
    val query = UsersTable.select(UsersTable.id, UsersTable.username, UsersTable.age)
      .where(UsersTable.id === fixedId).toCQL
  ...
  it should "support multiple WHERE predicates combined with and" in {
    val cql = UsersTable.select(UsersTable.age)
  ...
  it should "support ALLOW FILTERING" in {
    val cql = UsersTable.select(UsersTable.id)
  ...
  it should "render IN predicates" in {
    val cql = UsersTable.select(UsersTable.id, UsersTable.username)
  ...
  it should "reorder predicates to CQL key order" in {
    val cql = SensorsTable.select()
      .where(SensorsTable.ts > 100L and SensorsTable.year === 2026 and SensorsTable.deviceId === fixedId)
      .toCQL
