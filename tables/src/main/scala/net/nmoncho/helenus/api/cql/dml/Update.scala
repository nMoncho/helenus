 ...
 * WHERE parameters.
 */
final case class Update[T <: TableDef with Singleton, Eq <: HList, In <: HList, Rng <: HList, SetPm <: HList, WherePm <: HList](
    table: T,
  ...
  def set[V, P2 <: HList](assignment: table.BoundAssignment[V])(implicit
      @unused pp: Prepend.Aux[SetPm, V :: HNil, P2]
  ): Update[T, Eq, In, Rng, P2, WherePm] =
    new Update[T, Eq, In, Rng, P2, WherePm](
      table, assignments :+ assignment, predicates, ttlSeconds, timestampMicros, ifExistsFlag)
   ...
   * parameter list.
   */
  def where[P <: WhereClause, E <: HList, I <: HList, R <: HList, Pm <: HList, E2 <: HList, I2 <: HList, R2 <: HList, W2 <: HList](pred: P)(implicit
      ps: PredicateShape.Aux[P, E, I, R, Pm],
    ...
    new Update[T, E2, I2, R2, SetPm, W2](
      table, assignments, predicates ++ ps.predicates(pred), ttlSeconds, timestampMicros, ifExistsFlag)

  def usingTTL(seconds: Int): Update[T, Eq, In, Rng, SetPm, WherePm]       = copy(ttlSeconds = Some(seconds))
  def usingTimestamp(micros: Long): Update[T, Eq, In, Rng, SetPm, WherePm] = copy(timestampMicros = Some(micros))
   ...
   */
  def toFunction[AllPm <: HList, F](implicit
      @unused ev: CanUpdate[table.PK, table.CK, Eq, In, Rng],
