...
// hotels: ZCqlStream[Try[Hotel]] = zio.stream.ZStream@35ee56d6

val hotelById: ZCqlStream[Try[Hotel]] =
  "SELECT * FROM hotels WHERE id = ?".toZCQL.prepare[String].to[Hotel].stream("h1")
// hotelById: ZCqlStream[Try[Hotel]] = zio.stream.ZStream@636f7e8a
...
val insert =
  "INSERT INTO hotels(id, name, phone, address, pois) VALUES(?, ?, ?, ?, ?)".toZCQL
    ...
    .from[Hotel]
    .sink()
// insert: zio.stream.ZSink[ZCqlSession, Throwable, Hotel, Nothing, Unit] = zio.stream.ZSink@1e2023bf
```
