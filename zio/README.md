...
// hotels: ZCqlStream[Try[Hotel]] = zio.stream.ZStream@76153c52

val hotelById: ZCqlStream[Try[Hotel]] =
  "SELECT * FROM hotels WHERE id = ?".toZCQL.prepare[String].to[Hotel].stream("h1")
// hotelById: ZCqlStream[Try[Hotel]] = zio.stream.ZStream@5855f332
...
val insert =
  "INSERT INTO hotels(id, name, phone, address, pois) VALUES(?, ?, ?, ?, ?)".toZCQL
    ...
    .from[Hotel]
    .sink()
// insert: zio.stream.ZSink[ZCqlSession, Throwable, Hotel, Nothing, Unit] = zio.stream.ZSink@43937902
```
