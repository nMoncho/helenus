...
// session: CassandraSession = org.apache.pekko.stream.connectors.cassandra.scaladsl.CassandraSession@180b391e
...
// hotels: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(532198204)))

val hotelById: Source[Hotel, NotUsed] =
  "SELECT * FROM hotels WHERE id = ?".toCQLAsync.prepare[String].as[Hotel].asReadSource("h1")
// hotelById: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(102638169)))
...
// insert: Sink[Hotel, Future[Done]] = Sink(SinkShape(FlatMapPrefix.in(1881674782)))
```
