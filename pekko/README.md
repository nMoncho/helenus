...
// session: CassandraSession = org.apache.pekko.stream.connectors.cassandra.scaladsl.CassandraSession@5389774e
...
// hotels: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(1390151028)))

val hotelById: Source[Hotel, NotUsed] =
  "SELECT * FROM hotels WHERE id = ?".toCQLAsync.prepare[String].as[Hotel].asReadSource("h1")
// hotelById: Source[Hotel, NotUsed] = Source(SourceShape(FutureFlattenSource.out(355996314)))
...
// insert: Sink[Hotel, Future[Done]] = Sink(SinkShape(FlatMapPrefix.in(1954697102)))
```
