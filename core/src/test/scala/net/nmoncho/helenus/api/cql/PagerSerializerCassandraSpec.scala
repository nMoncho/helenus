...
/** End-to-end paging round-trip against a genuine driver paging state, plus the
  * `DefaultPagingStateSerializer` rejection cases that only a real (validating)
  * state can exercise. The pure `serialize`/`deserialize` behaviour of both
  * serializers is covered without Cassandra in `PagerSerializerSpec`.
  */
class PagerSerializerCassandraSpec
    extends AnyWordSpec
    with Matchers
    with Eventually
    with CassandraSpec
  ...
  "DefaultPagingStateSerializer" should {
    ...
    "react to a state from another statement" in {
      val queryHotels = "SELECT * FROM hotels".toCQL.prepareUnit.as[Hotel]
      val queryRooms  =
    ...
    "react to state from same statement but different parameters" in {
      val queryRooms =
        "SELECT date, room_number, is_available FROM available_rooms_by_hotel_date WHERE hotel_id = ?".toCQL
          .prepare[String]
          .as[(LocalDate, Short, Boolean)]
  ...
  private def validateRoundTrip[A]()(implicit ser: PagerSerializer[A]): Unit =
    "serialize state" in {
      val queryHotels = "SELECT * FROM hotels".toCQL.prepareUnit.as[Hotel]
